...
def get_safescad_data(dataset_file, data_dir, noise):
    ...
    if dataset_file is not None:
        ...
        X_train, X_calibration, y_train, y_calibration = train_test_split(X_train, y_train, test_size=0.20, stratify=y_train,
                                                            random_state=random_state)
        X_confusion, X_test, y_confusion, y_test = train_test_split(X_test, y_test, test_size=0.50, stratify=y_test,
                                                            random_state=random_state)
        ...
        X_calibration_enc = scaler.transform(X_calibration.values)
        X_confusion_enc = scaler.transform(X_confusion.values)
        ...
        y_calibration_enc = onehot.transform(y_calibration.to_numpy().reshape(-1,1)).toarray()
        y_confusion_enc = onehot.transform(y_confusion.to_numpy().reshape(-1,1)).toarray()
        y_test_enc = onehot.transform(y_test.to_numpy().reshape(-1,1)).toarray()

        print("Saving train, calibration, confusion, and test data ...")
        save_data(X_train_enc, y_train_enc, feature_names, onehot, 'TOT', 'train', data_dir)
        save_data(X_calibration_enc, y_calibration_enc, feature_names, onehot, 'TOT', 'calibration', data_dir)
        save_data(X_confusion_enc, y_confusion_enc, feature_names, onehot, 'TOT', 'confusion', data_dir)
        save_data(X_test_enc, y_test_enc, feature_names, onehot, 'TOT', 'test', data_dir)
        save_encoders(scaler, onehot, data_dir)

        return (X_train_enc, y_train_enc, X_calibration_enc, y_calibration_enc, X_confusion_enc, y_confusion_enc, X_test_enc, y_test_enc)

    else:
        train_data = pd.read_csv(f'{data_dir}/train.csv')
        calibration_data = pd.read_csv(f'{data_dir}/calibration.csv')
        confusion_data = pd.read_csv(f'{data_dir}/confusion.csv')
        test_data = pd.read_csv(f'{data_dir}/test.csv')

        y_train_enc = train_data[['TOT_fast', 'TOT_med_fast', 'TOT_med', 'TOT_med_slow', 'TOT_slow']]
        y_calibration_enc = calibration_data[['TOT_fast', 'TOT_med_fast', 'TOT_med', 'TOT_med_slow', 'TOT_slow']]
        y_confusion_enc = confusion_data[['TOT_fast', 'TOT_med_fast', 'TOT_med', 'TOT_med_slow', 'TOT_slow']]
        ...
        X_calibration_enc = calibration_data.drop(['Unnamed: 0', 'TOT_fast', 'TOT_med_fast', 'TOT_med', 'TOT_med_slow', 'TOT_slow'], axis=1)
        X_confusion_enc = confusion_data.drop(['Unnamed: 0', 'TOT_fast', 'TOT_med_fast', 'TOT_med', 'TOT_med_slow', 'TOT_slow'], axis=1)
        ...
        return (X_train_enc.values, y_train_enc.values, X_calibration_enc.values, y_calibration_enc.values,
                X_confusion_enc.values, y_confusion_enc.values, X_test_enc.values, y_test_enc.values)
...
def get_safescad2_data(dataset_file, data_dir, noise):
    ...
    if dataset_file is not None:
        ...
        save_data(X_calibration, y_calibration, feature_names, onehot, 'TOT', 'calibration', data_dir)
        save_data(X_confusion, y_confusion, feature_names, onehot, 'TOT', 'confusion', data_dir)
        save_data(X_test, y_test, feature_names, onehot, 'TOT', 'test', data_dir)

        return (X_train, y_train, X_calibration, y_calibration, X_confusion, y_confusion, X_test, y_test)
        ...
        y_calibration_enc = calibration_data[['TOT_0', 'TOT_1', 'TOT_2']]
        y_confusion_enc = confusion_data[['TOT_0', 'TOT_1', 'TOT_2']]
        y_test_enc = test_data[['TOT_0', 'TOT_1', 'TOT_2']]

        X_train_enc = train_data.drop(['Unnamed: 0', 'TOT_0', 'TOT_1', 'TOT_2'], axis=1)
        X_calibration_enc = calibration_data.drop(['Unnamed: 0', 'TOT_0', 'TOT_1', 'TOT_2'], axis=1)
        X_confusion_enc = confusion_data.drop(['Unnamed: 0', 'TOT_0', 'TOT_1', 'TOT_2'], axis=1)
...
def get_collision_data(dataset_file, data_dir, noise):
    print('Getting collision dataset...')
    n_categories = 2
    if dataset_file is not None:
        " Import Dataset """

        raw_columns = ['X-Dist', 'Y-Dist', 'Speed', 'Direction', 'RotSpeed', 'Collision']
        raw_df = pd.read_csv(dataset_file, names=raw_columns, delim_whitespace=True)
        ...
        # split features and targets
        y = df.Collision
        y = y.replace('noCollision','N')
        y = y.replace('collision','Y')
        ...
        # prepare encoders
        onehot = prepare_target(y, categories=[['Y', 'N']])

        # split train and test data
        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.40, stratify=y,
                                                            random_state=random_state)
        X_train, X_calibration, y_train, y_calibration = train_test_split(X_train, y_train, test_size=0.20,
                                                                          stratify=y_train,
                                                                          random_state=random_state)
        X_confusion, X_test, y_confusion, y_test = train_test_split(X_test, y_test, test_size=0.50, stratify=y_test,
                                                                    random_state=random_state)
        ...
        X_confusion = X_confusion.values
        ...
        print("Saving train, calibration, confusion, and test data ...")
        save_data(X_train, y_train_enc, feature_names, onehot, 'Collision', 'train', data_dir)
        save_data(X_calibration, y_calibration_enc, feature_names, onehot, 'Collision', 'calibration', data_dir)
        save_data(X_confusion, y_confusion_enc, feature_names, onehot, 'Collision', 'confusion', data_dir)
        save_data(X_test, y_test_enc, feature_names, onehot, 'Collision', 'test', data_dir)
        save_encoders(None, onehot, data_dir)

        return (X_train, y_train_enc, X_calibration, y_calibration_enc,
                X_confusion, y_confusion_enc, X_test, y_test_enc)
        ...
        y_train_enc = train_data[['Collision_Y', 'Collision_N']]
        y_calibration_enc = calibration_data[['Collision_Y', 'Collision_N']]
        y_confusion_enc = confusion_data[['Collision_Y', 'Collision_N']]
        y_test_enc = test_data[['Collision_Y', 'Collision_N']]

        X_train_enc = train_data.drop(['Unnamed: 0', 'Collision_Y', 'Collision_N'], axis=1)
        X_calibration_enc = calibration_data.drop(['Unnamed: 0', 'Collision_Y', 'Collision_N'],axis=1)
        X_confusion_enc = confusion_data.drop(['Unnamed: 0', 'Collision_Y', 'Collision_N'], axis=1)
        X_test_enc = test_data.drop(['Unnamed: 0', 'Collision_Y', 'Collision_N'], axis=1)
...
def get_data(experiment, dataset_file, data_dir, noise=None):
    print("Loading and preprocessing data ...")
    get_data = globals()[f'get_{experiment}_data']
    return get_data(dataset_file, data_dir, noise)
