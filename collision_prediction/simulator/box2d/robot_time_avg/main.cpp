...
int main(int argc, char** argv)
{
    ...
    while(!nc_done || !c_done)
    {
        ...
        if (!collision && !nc_done)
        {
            double nc_time_sum_new=nc_time_sum+t;
            ++nc;
            if (nc>5000 && (nc_time_sum/(nc-1) - nc_time_sum_new/nc)*(nc_time_sum/(nc-1) - nc_time_sum_new/nc)<=0.01) nc_done=1;
            nc_time_sum+=t;
        }
        else if (collision && !c_done)
        {
          double c_time_sum_new=c_time_sum+t;
          ++c;  
          if (c>5000 && (c_time_sum/(c-1) - c_time_sum_new/c)*(c_time_sum/(c-1) - c_time_sum_new/c)<=0.01) c_done=1;
          c_time_sum+=t;
        } 
    }

    printf("Average travel time: %f\n", nc_time_sum/nc);
    printf("Average collision travel time: %f\n", c_time_sum/c);
