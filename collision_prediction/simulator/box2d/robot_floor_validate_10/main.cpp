...
int main(int argc, char** argv)
{
    ...
    char const * partStrTempCollisionDataPath=argv[3];
    char const * partStrTempNoCollisionDataPath=argv[4];
    char const * col_pred_path=argv[5];
    char const * no_col_pred_path=argv[6];

    // Number of waypoints
    int visit=std::stoi(argv[7]);
    ...
    std::string outfile_path(argv[12]);
    ...
    while(visited<visit)
    {
        ...
        while (ind_start==ind_end) ind_end=unif_p(rng)*waypoints.size();
        // printf("start and end initialised\n");
        ...
        // printf("Starting transformations\n");
        ...
        for(int i=1; i<path.size(); ++i)
        {
            ...
            while (!travel)
            {
                ...
                if(unif_p(rng)<p_cldr_presence)
                {
                    ...
                    // For one verify
                    // V1
                    if (label==1 && v2==0) p_x=x0;
                    else if (label==1 && v2==1) p_x=x1;
                    else if (label==0 && v2==0) p_x=x2;
                    else if (label==0 && v2==1) p_x=x3;
            ...
            while (!reached)
            {
                ...
                if (t>5e3) rbt.body->SetTransform(b2Vec2(x_target,y_target),0);
            ...
            time_vec.push_back(t/60);
        }
        // printf("Finished journey\n");
    }

    cout<<"Finished journeys" <<endl;
    ...
    // V1 
    std::string filename=outfile_path+"/simulate_v1_"+std::to_string(visit)+".txt";
    ...
    outfile << x0 << " " << x1 << " " << x2 << " " << x3 << " " << time_avg << " " << time_stdev << " " << 1-risk_avg << " " << risk_stdev<<std::endl;    
