...
// - datasize: Maximum number of samples (stop if c_datasize+nc_datasize>datasize)
// - valid: If true then it must be collision/no-collision 100 consective runs 

int main(int argc, char** argv)
{
    // char const * outfile_path=argv[1];
    std::string outfile_path(argv[1]);
    int datasize=std::stoi(argv[2]);

    std::ofstream outfile_collision;
    outfile_collision.open((outfile_path+"/collisions.txt").c_str());
    std::ofstream outfile_no_collision;
    outfile_no_collision.open((outfile_path+"/no_collisions.txt").c_str());    
    ...
    while(c<datasize || nc<datasize)
    {
        ...
        while(succ_count<sanity && valid_pick==1)
        {
            ...
            {
                if (val==-1) val=0;
                else if(val==1) valid_pick=0;
            }
            else
            {
                if (val==-1) val=1;
                else if(val==0) valid_pick=0;
            }
            ++succ_count;
        }
        ...
        if (valid_pick)
        {
            if (val && nc<datasize && valid_pick)
            {
              ++nc;
              outfile_no_collision << data << std::endl;
            }
            else if (!val && c<datasize && valid_pick)
            {
              ++c;  
              outfile_collision << data << std::endl;
            } 
        }
